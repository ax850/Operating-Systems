...
test_basic()
{
	...
	//printf("ID is :%d\n",ret2);
	ret = thread_kill(ret2); // Destroying first user created thread
	minfo = mallinfo();
...
test_wakeup(int all)
{
	...
	/* create all threads */
	for (ii = 0; ii < NTHREADS; ii++) {
		child[ii] = thread_create((void (*)(void *))test_wakeup_thread,		// Create new Threads and interrupt_handler calls thread_yeild to run them
					  (void *)ii);
		assert(thread_ret_ok(child[ii]));
	}
out:
	while (__sync_fetch_and_add(&done, 0) < NTHREADS) {
		...
		/* tests thread_wakeup */
		assert(interrupts_enabled());
		ret = thread_wakeup(queue, all);		//all is 0
		assert(interrupts_enabled());
		assert(ret >= 0);
